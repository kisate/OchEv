    ...
    fun replaceVertex(old: VertexFigure, new: VertexFigure) {
        ...
        graph.figures.edges.forEach {
            if (it.first.beginFigure == old) {
                when (it.first) {
                    is Line -> newGraph.figures.addEdge(Line(new, it.first.endFigure), it.second)
                }
            } else if (it.first.endFigure == old) {
                when (it.first) {
                    is Line -> newGraph.figures.addEdge(Line(it.first.endFigure, new), it.second)
                }
            } else {
